...
from apps.utils.models.query import Q, get_single_object_or_none
...
class ConfigurationAutocomplete(autocomplete.Select2QuerySetView):
    ...
    def get_queryset(self):
        ...
        if section := get_single_object_or_none(Section, pk=self.forwarded.get("section")):
        ...
        if app := apps.app_configs.get(self.forwarded.get("app_label")):
            expected_content_type = ContentType.objects.get_for_model(apps.get_model(app.configuration_model))
...
class AppAutocomplete(autocomplete.Select2ListView):
    ...
    def get_list(self):
        ...
        return [(p.label, p.verbose_name) for p in all_plugin_apps() if p.label not in exclude]
