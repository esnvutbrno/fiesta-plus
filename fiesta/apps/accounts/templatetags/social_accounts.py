...
def get_social_icon(provider: str) -> str:
    if provider == 'esnaccounts':
        ...
        return mark_safe('''<svg class="w-6 h-6 mr-2 -ml-1" viewBox="0 0 40 40">
                      <use href="/static/esn-star-color.svg#esn-star-color"></use>
                  </svg>''')

    elif provider == 'facebook':
        return mark_safe('''<svg class="mr-2 -ml-1 w-4 h-4"
                       aria-hidden="true"
                       focusable="false"
                       role="img"
        ...
        return mark_safe('''<svg class="mr-2 -ml-1 w-5 h-5"
                       aria-hidden="true"
                       focusable="false"
                       role="img"
                       xmlns="http://www.w3.org/2000/svg"
...
@register.simple_tag
def get_social_button_style(provider: str) -> str:
    print(provider)
    if provider == 'esnaccounts':
        return '''text-black bg-[#DDDDDD] hover:bg-[#DDDDDD]/90'''

    elif provider == 'facebook':
        return '''text-white bg-[#3b5998] hover:bg-[#3b5998]/90'''

    elif provider == 'google':
        return '''text-white bg-[#4285F4] hover:bg-[#4285F4]/90'''

    elif provider == 'apple':
        return '''text-white bg-[#050708] hover:bg-[#050708]/90'''
